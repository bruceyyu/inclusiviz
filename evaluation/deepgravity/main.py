...
parser.add_argument('--experiment-visitors', type=int, default=1, help='experiment: whether add visitor characteristics')
parser.add_argument('--experiment-round', type=int, default=-1, help='experiment: the current round index')
...
print("args.experiment_visitors", args.experiment_visitors)
...
def train(epoch):
    ...
    for batch_idx, data_temp in enumerate(train_loader):
        ...
        for data, target in zip(b_data, b_target):
            ...
            # print(ss_data.shape, target.shape)
            if args.cuda:
...
train_dataset_args = {'tileid2oa2features2vals': tileid2oa2features2vals,
                      ...
                      'frac_true_dest': 0.9,
                      'model': model_type,
                      'experiment_visitors': args.experiment_visitors}

total_dataset_args = {'tileid2oa2features2vals': tileid2oa2features2vals,
                      'o2d2flow': o2d2flow,
                      'oa2features': oa2features,
                      'oa2pop': oa2pop,
                      'oa2centroid': oa2centroid,
...
total_dataset = dgd.FlowDataset(train_data + test_data, **total_dataset_args)
...
if args.mode == 'train':
    ...
    if args.experiment_round != -1:
        remarks.append(f"round_{args.experiment_round}")
    if args.experiment_visitors == 0:
        remarks.append(f"no_visitors")
    
    if len(remarks) > 0:
        fname = './results/model_{}_{}_{}_{}_log.pt'.format(model_type, args.dataset, args.selected_subgroup_cate, "_".join(remarks))
    else:
