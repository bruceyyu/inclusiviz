...
class FlowDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.experiment_visitors = experiment_visitors
    ...
    def get_all_features(self):
        all_feat = np.array([self.__getitem__(i)[0][0].numpy() for i in range(len(self.list_IDs))])
        return all_feat.reshape(all_feat.shape[0] * all_feat.shape[1], all_feat.shape[2])
    ...
    def get_features(self, oa_origin, oa_destination):
        ...
        race_o = np.array(self.oa2features[oa_origin][1:5])
        race_inflow_o = np.array(self.oa2features[oa_origin][5:9])
        race_inflow_d = np.array(self.oa2features[oa_destination][5:9])
        # print(self.oa2features[oa_destination], oa_destination, race_inflow_d)
        ...
        party_inflow_o = np.array(self.oa2features[oa_origin][32:34])
        party_d = np.array(self.oa2features[oa_destination][29:31])
        party_inflow_d = np.array(self.oa2features[oa_destination][32:34])

        poi_o = self.oa2features[oa_origin][17:29]
        poi_d = self.oa2features[oa_destination][17:29]

        dist_od = utils.earth_distance(oa2centroid[oa_origin], oa2centroid[oa_destination])
        if self.experiment_visitors == 1:
            return np.concatenate((pop_o, pop_d, poi_o, poi_d, race_inflow_d, income_inflow_d, [dist_od]))
            # return np.concatenate((pop_o, pop_d, poi_o, poi_d, income_inflow_d, party_inflow_d, [dist_od]))
        else:
            return np.concatenate((pop_o, pop_d, poi_o, poi_d, [dist_od]))
        # return np.concatenate((pop_o, pop_d, poi_d, income_inflow_d, party_inflow_d, [dist_od]))
    ...
    def get_destinations(self, oa, size_train_dest, all_locs_in_train_region):
        ...
        if size_fake_dests > 0 and len(fake_dests_all) > 0:
            ...
            fake_dests = np.array([])
        # print("true_dests", len(true_dests), "fake_dests", len(fake_dests), "all", len(true_dests_all))
