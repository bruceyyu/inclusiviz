...
model_path = 'od_models.py'
od = SourceFileLoader('od', os.path.join(dir_path, model_path)).load_module()
...
def get_features_ffnn(oa_origin, oa_destination, oa2features, oa2centroid, df, distances, k):
    ...
    if df == 'deepgravity':
        ...
        return np.concatenate((np.log(oa2features[oa_origin]), np.log(oa2features[oa_destination]), [dist_od]))
